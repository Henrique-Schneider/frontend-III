...
function Lista() {
  const cidadesBrasil = cidades.filter((cidade) => cidade.country === "BRA");
  return (
    <div>
      <h1>Lista de Cidades Brasileiras</h1>
      <div className={styles.container}>
        {cidadesBrasil.map((cidade) => (
          <div
            key={cidade.id}
            style={{ backgroundColor: cidade.color }}
            className="cidade-card"
          >
            <h2>ID: {cidade.id}</h2>
            <h3>Cidade: {cidade.city}</h3>
            <p>País: {cidade.country}</p>
            <p>População: {cidade.population}</p>
          </div>
        ))}
      </div>
    </div>
